...
class ModelTrainer:
    def __init__(self,d="./data/allT20/",o=20,date=datetime(2015, 5, 7)):
        ...
        if self.o==50: #odi matches
            ...
            self.wicket_segments=np.array([[0,0,0],[0,2,10],[0,4,10],[0,5,10],[0,7,10]])/10.
        ...
        col=["balls_remain","rr","rrr", "sr","extras_striker","extras_bowler","e_r_career","w_b","striker_performance","wickets_remain","s_r_career","Av_career"]
        game_table[col]=game_table[col].dropna()
        for c in col:
            game_table[c]=(game_table[c]-np.min(game_table[c]))/(np.max(game_table[c])-np.min(game_table[c]))
        game_table.to_csv(d+"/stats/game_table_norm.csv")
        ...
        ze = game_table[game_table.runs_off_bat == 0].iloc[0]
        ...
        self.add = self.add.append([ze, th, fo, fi, si,wso,eso,eso2])
        ...
        self.var_r1 = ["balls_remain", "wickets_remain", "rr", "sr", "w_b",'s_r_career',"striker_performance"]
        ...
        self.var_r2 = ["balls_remain", "wickets_remain", "rrr", "sr", "w_b",'s_r_career',"striker_performance"]
    ...
    def test_inning(self,ini):
        ...
        for i in range(1, len(self.over_segments)):
            ...
            for j in range(1, len(self.wicket_segments[i])):
                ...
                print()
                total_constant.append(mean_squared_error(df_segment_ij.runs_off_bat, [(base*np.array([0,1,2,3,4,5,6])).sum() for i in range(len(df_segment_ij))]))
                total_avg.append(mean_squared_error(df_segment_ij.runs_off_bat, [(avg_probs_runs*np.array([0,1,2,3,4,5,6])).sum() for i in range(len(df_segment_ij))]))
                total_model.append(mean_squared_error(df_segment_ij.runs_off_bat, (pred*np.array([0,1,2,3,4,5,6])).sum(axis=1)))
                print("Base:", mean_squared_error(df_segment_ij.runs_off_bat, [(base*np.array([0,1,2,3,4,5,6])).sum() for i in range(len(df_segment_ij))]))
                print("Average:",
                      mean_squared_error(df_segment_ij.runs_off_bat, [(avg_probs_runs*np.array([0,1,2,3,4,5,6])).sum() for i in range(len(df_segment_ij))]))
                print("Model:", mean_squared_error(df_segment_ij.runs_off_bat, (pred*np.array([0,1,2,3,4,5,6])).sum(axis=1)))
            print(sum(np.array(total_constant))/(len(self.wicket_segments[0])*len(self.over_segments)))
            print(sum(np.array(total_avg)) / (len(self.wicket_segments[0]) * len(self.over_segments)))
            print(sum(np.array(total_model)) / (len(self.wicket_segments[0]) * len(self.over_segments)))
