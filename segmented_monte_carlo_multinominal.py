...
class SegmentedMonteCarlo:
    def __init__(self,d="./data/T20I/",num_iter=1000):
        self.d=d
    ...
    def monte_carlo(self,r,b,w,rr,so,st,players_wb,players_er,players_sr,players_av,player_performance,extras_strikers,extras_bowlers,ini=1,seg=4):
        if ini==1:
            s_model=self.sizes
            os_model=self.os
            ps_model=self.ps
        ...
        while con:
            ...
            pf=player_performance[p_striker*f+p_partner*(1-f)]
            ...
            if ini==1:
                e_o=es_model[seg]
                e = 0.0528#e_o.predict([[1, b/120, w/10, rr/3.6, (f * so + (1 - f) * st)/172, e_s, eb]])[0]
                #print(e)
                m_o=os_model[seg]
                #o=m_o.predict_proba([[b,w,rr,f*so+(1-f)*st,wb,er]])[0]
                o = m_o.predict([[1, b/120, w/10, rr/3.6, (f * so + (1 - f) * st)/172, er]])[0]
                #print(o)
                p_o=ps_model[seg]
                #p=p_o.predict_proba([[b,w,rr,f*so+(1-f)*st,wb,av,sr]])[0]
                #print([b, w, rr, f * so + (1 - f) * st, wb, sr, pf])
                p = p_o.predict([[b/120, w/10, rr/3.6, (f * so + (1 - f) * st)/172, wb, sr, pf]])[0]
                #print(p)
                #print(b)
            else:
                e_o = es_model[seg]
                e = e_o.predict([[b, w, rrr, f * so + (1 - f) * st, e_s, eb,1]])
                # print(e)
                m_o = os_model[seg]
                #o = m_o.predict_proba([[b, w, rrr, f * so + (1 - f) * st, wb, er]])[0]
                o = m_o.predict([[b, w, rrr, f * so + (1 - f) * st, er,1]])
                # print(o)
                p_o = ps_model[seg]
                #p = p_o.predict_proba([[b, w, rrr, f * so + (1 - f) * st, wb, av, sr]])[0]
                p = p_o.predict([[b, w, rrr, f * so + (1 - f) * st, wb, sr, pf]])[0]
                # print(p)
            if rand<e:
                #print(e)
